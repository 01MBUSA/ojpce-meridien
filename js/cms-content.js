(function () {
  ...
  const cmsStyle = document.createElement("style");
  cmsStyle.textContent = `
    .project-project-gallery{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px;width:100%;padding:8px;box-sizing:border-box}
    .cms-project-photo{width:100%;min-height:180px;border:0;border-radius:8px;background-size:cover;background-position:center;background-repeat:no-repeat;cursor:pointer;padding:0;display:block}
    .cms-project-empty{min-height:180px;border-radius:8px;background:#f1f4f1;color:#7b877f;display:flex;flex-direction:column;align-items:center;justify-content:center;font-size:38px}
    .cms-project-empty span{font:600 12px Montserrat,sans-serif;margin-top:6px}
    .project-card{display:block !important}
    .project-card .project-project-gallery{margin-bottom:12px}
    ...
    try {
      ...
      areas.forEach(area => {
        ...
        area.innerHTML = (data.items || [])
          .slice(0, home ? 3 : 999)
          .map(item => {
            const rawImages = [item.image, ...(Array.isArray(item.images) ? item.images : [])];
            const images = rawImages.filter(Boolean).map(asset).filter(Boolean)
              .filter((image, i, arr) => arr.indexOf(image) === i);
            const photos = images.map((image, index) => `
              <button type="button" class="cms-project-photo"
                data-images="${esc(JSON.stringify(images))}" data-index="${index}"
                aria-label="Agrandir la photo ${index + 1}"
                style="background-image:url('${esc(image)}')"></button>
            `).join("");
            const gallery = photos || `<div class="cms-project-empty" role="img" aria-label="Aucune photo disponible">📷<span>Aucune photo publiée</span></div>`;
            ...
            return home
              ...
              : `<article class="project-large">
                  ...
                  <div class="project-text">
                    <span class="meta">${esc([item.location, item.category].filter(Boolean).join(" · "))}</span>
                    <h3>${esc(item.title)}</h3>
                    ${item.description ? `<p>${esc(item.description)}</p>` : ""}
                    ${item.body ? `<div class="project-detail">${esc(item.body).replace(/\n/g,"<br>")}</div>` : ""}
                    <a class="text-btn" href="contact.html">Participer au projet →</a>
                  </div>
                </article>`;
          }).join("");
      });
    } catch (error) { console.warn("Projets CMS:", error); }
  }
